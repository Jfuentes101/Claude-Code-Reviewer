...
class CiWatch:
    ...
    async def _one(self, repo: RepoConfig, row: sqlite3.Row) -> Outcome | None:
        ...
        try:
            result = await publish.once_per(
                self.db,
                publish.posted_key("approved-red", repo, meta),
                lambda: publish.report_red_build(repo, meta, red, dry_run=quiet),
                quiet=quiet,
            )
        except GhError as ex:
        ...
        self._settle(row["key"], outcome)
        if result is None:  # the author already has this note
            return None
    ...
    def _settle(self, key: str, state: str) -> None:
        """A quiet pass decides but must not settle it: settling a red build whose
        note never went out leaves the author owed a comment nobody will post."""
        if not (self.dry_run or self.no_publish):
